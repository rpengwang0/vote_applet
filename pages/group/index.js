...
Page({
  ...
  playVideo: function (e) {
    ...
    var that = this;
    let video = that.data.playerArray;

    for (var i in video) {
        var index_video = 'video_' + video[i].player_id;
    ...
    onShareAppMessage: function () {
        ...
        let url = 'pages/group/index?activityId=' + that.data.activityId;
        ...
        if (that.data.group_name.length > 0) {
    ...
    onShareTimeline: function () {
      ...
      return {
          title: that.data.activityInfo.activity_title,      
          imageUrl: that.data.activityInfo.share_img,
          query: 'activityId='+ that.data.activityInfo.activity_id// 分享路径
      }
    },
    /**
     * 页面相关事件处理函数--监听用户下拉动作
    ...
    homeback: function () {
        let url = '/pages/vote/vote?activityId=' + this.data.activityId + '&has_group=' + this.data.has_group;
        wx.redirectTo({
            url: url,
        })
    },
    ...
    onShow: function () {
        ...
        wx: wx.getSystemInfo({
            success: function (res) {
                var version = res.version.substr(res.version.length - 1, 1);
                console.log(version);
                if (version > num) {
                    wx.hideHomeButton({
                        success: function () {
